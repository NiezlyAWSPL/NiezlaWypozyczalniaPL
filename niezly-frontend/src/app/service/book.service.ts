...
export class BookService {
  ...
  getBook(bookId: number): Observable<BookDTO> {
    return this.http.get<BookDTO>(`${Constants.API_URL}/books/${bookId}`, {headers: this.sessionService.getHeaders()});
  }

  createBook(createBookRequestDTO: CreateBookRequestDTO): Observable<BookDTO> {
    return this.http.post<BookDTO>(`${Constants.API_URL}/books`, createBookRequestDTO, {headers: this.sessionService.getHeaders()});
  }

  getFirstAvailableByBookDefinition(bookDefinitionId: number): Observable<BookDTO> {
    return this.http.get<BookDTO>(`${Constants.API_URL}/books`, {
  ...
  getBookDefinitions(bookDefinitionFilter: BookDefinitionFilterDTO): Observable<BookDefinitionDTO[]> {
    return this.http.post<BookDefinitionDTO[]>(`${Constants.API_URL}/books/definitions/filtered`, bookDefinitionFilter, {headers: this.sessionService.getHeaders()});
  }

  getFilteredBooks(titlePhase: string): Observable<BookDTO[]> {
    return this.http.get<BookDTO[]>(`${Constants.API_URL}/books/filter/{${titlePhase}`, {headers: this.sessionService.getHeaders()});
  }
}
